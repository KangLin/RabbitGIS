...
#   然后再进入脚本目录：`cd ${RabbitGISRoot}/ThirdLibrary/build_script`。再运行你想运行的编译脚本。例如： `./build.sh` 
...
#需要设置下面变量：
#QT_ROOT=/c/Qt/Qt5.7.0-msvc/5.7/msvc2015 #QT 安装根目录,默认为:${RabbitGISRoot}/ThirdLibrary/windows_msvc/qt
...
if [ -n "${RabbitGISRoot}" ]; then
    RABBITGIS_BUILD_PREFIX=${RabbitGISRoot}/windows_msvc
else
    RABBITGIS_BUILD_PREFIX=`pwd`/../windows_msvc    #修改这里为安装前缀 
fi
